/**
 * EventKraft AI Assistant — chat logic shared by the floating widget and the
 * full-page assistant (/assistant). Each [data-ek-ai] root is initialised
 * independently; history persists in sessionStorage so it carries across the
 * widget, the full page and page navigation.
 */
(function () {
  ...
  function initAi(root) {
    const toggle = root.querySelector('.ek-ai-toggle');     // widget only
    const panel = root.querySelector('.ek-ai-panel');
    const minimizeBtn = root.querySelector('.ek-ai-minimize'); // widget only
    const clearBtn = root.querySelector('.ek-ai-clear');    // full page only
    const messagesEl = root.querySelector('.ek-ai-messages');
    const form = root.querySelector('form');
    const input = root.querySelector('.ek-ai-input');
    const sendBtn = root.querySelector('.ek-ai-send');
    if (!panel || !messagesEl || !form || !input || !sendBtn) return;
    ...
    if (minimizeBtn) {
      minimizeBtn.addEventListener('click', function () { panel.hidden = true; });
    }
    ...
    // Panels that are visible on load (e.g. the full page) render immediately.
    if (!panel.hidden) {
      renderAll();
      scrollToBottom();
    }
  ...
  document.querySelectorAll('[data-ek-ai]').forEach(initAi);
})();
